...
public class MelodyShape
{
	...
	public static MelodyComparer getComparer(String aOpt, MelodyCollection coll){
		if (aOpt.equals("2010-domain"))
			return new NGramMelodyComparer(3, new HybridAligner(
					new FrequencyNGramComparer(coll, 3, new IntervalPitchNGramComparer()))); // faster without cache
		else if (aOpt.equals("2010-pitchderiv"))
			return new NGramMelodyComparer(3, new HybridAligner(new CachedNGramComparer(
			...
			return new NGramMelodyComparer(4, new HybridAligner(new CachedNGramComparer(
			...
			return new NGramMelodyComparer(3, new LocalAligner(new CachedNGramComparer(
					new FrequencyNGramComparer(coll, 3, new BSplineShapeNGramComparer(8, 1, 0.5)))));
		else if (aOpt.equals("2012-shapeg"))
			return new NGramMelodyComparer(3, new GlobalAligner(new CachedNGramComparer(
					new FrequencyNGramComparer(coll, 3, new BSplineShapeNGramComparer(8, 1, 0.5)))));
		else if(aOpt.equals("2012-shapetime") || aOpt.equals("2013-shapetime"))
			return new NGramMelodyComparer(4, new HybridAligner(new CachedNGramComparer(
					new CombinedNGramComparer(
							new BSplinePitchNGramComparer(), 1, 2.1838,
							new BSplineTimeNGramComparer(), 0.5, 0.4772))));
		else 
			throw new IllegalArgumentException("unrecognized algorithm name: " + aOpt);
	}
	
	public static ResultRanker getRanker(String aOpt, MelodyCollection coll){
		if (Arrays.asList("2010-domain", "2010-pitchderiv", "2010-shape", "2011-shape", "2012-shapeh", "2012-shapel", "2012-shapeg", "2013-shapeh").contains(aOpt))
	...
	public static Result[] runAlgorithm(MelodyComparer melodyCmp, MelodyComparer melodyCmpRerank, ResultRanker ranker, ResultRanker rankerRerank,
			int kOpt, ArrayList<Melody> queries, int numQuery, MelodyCollection coll, int tOpt, UIObserver observer){		
	 ...
	 */
	public static Result[] runComparer(final MelodyComparer melodyCmp, final Melody query, final Iterable<Melody> coll, final int collSize,
			final int numQuery, final int totalQueries, int tOpt, final UIObserver observer) {		
